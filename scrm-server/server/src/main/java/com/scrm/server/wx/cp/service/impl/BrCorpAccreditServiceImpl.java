...
public class BrCorpAccreditServiceImpl extends ServiceImpl<BrCorpAccreditMapper, BrCorpAccredit> implements IBrCorpAccreditService {
    ...
    public BrCorpAccredit getByCorpId(String corpId) {
        ...
        try {
            ...
            if (ListUtils.isNotEmpty(accreditList)) {
                return accreditList.get(0);
            }
            // SaaS模式：找不到返回null，由调用方处理
            // Private模式：构造默认对象
            if (ScrmConfig.isPrivateMode()) {
                BrCorpAccredit defaultAccredit = new BrCorpAccredit();
                defaultAccredit.setCorpId(ScrmConfig.getExtCorpID());
                return defaultAccredit;
            }
            return null;
