...
const shouldTransferCustomer = (record) => {
  const { staffTransferInfo, extCreatorId } = record;
  if (staffTransferInfo) {
    const isCurrentUser = staffTransferInfo.handoverStaffExtId === extCreatorId;
    return isCurrentUser
      ? staffTransferInfo.status !== HANDLE_STATUS.WAIT
      : true;
  } else {
    return true;
  }
...
export default (props) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => {
        if (record.staffTransferInfo) {
          const isCurrentUser = record.staffTransferInfo.handoverStaffExtId === record.extCreatorId;
          return isCurrentUser ? handleHandleStatus(record.staffTransferInfo): '';
        } else {
          return "";
        }
      },
    },
    {
      ...
      title: "标签",
      render: (val, record) => {
        const { followStaffList = [], extCreatorId } = record;
        const followStaffItem = Array.isArray(followStaffList)
          ? followStaffList.find((ele) => ele.extId === extCreatorId)
          : null;
        if (!followStaffItem || !followStaffItem.tags?.length) {
          return "-";
        }
        return <TagCell dataSource={followStaffItem.tags} />;
      },
    },
    {
      dataIndex: "createdAt",
  ...
  return (
    <CommonDrawer footer={false} visible={visible} {...rest} width={1000}>
      ...
      <TableContent
        ...
        rowKey="id"
        columns={columns}
        name={selectedKeys.length ? `已选择${selectedKeys.length}条数据` : null}
